...
impl MoonshineModel {
    pub fn new(
        ...
        let encoder = Self::init_session(&encoder_path, execution)?;

        log::info!("Loading Moonshine decoder from {:?}...", decoder_path);
        let decoder = Self::init_session(&decoder_path, execution)?;
    ...
    fn init_session(
        ...
        log::info!(
            "Moonshine session provider selection: {:?} (cuda_available={}, reason={})",
