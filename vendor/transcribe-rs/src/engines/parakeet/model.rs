...
use crate::onnx_execution::{resolve_onnx_execution, OnnxExecutionParams};
...
impl ParakeetModel {
    pub fn new<P: AsRef<Path>>(
        model_dir: P,
        quantized: bool,
        execution: &OnnxExecutionParams,
    ) -> Result<Self, ParakeetError> {
        let encoder = Self::init_session(&model_dir, "encoder-model", None, quantized, execution)?;
        ...
        let preprocessor = Self::init_session(&model_dir, "nemo128", None, false, execution)?;
    ...
    fn init_session<P: AsRef<Path>>(
        ...
        let resolution = resolve_onnx_execution(execution);
        log::info!(
            "Parakeet session '{}' provider selection: {:?} (cuda_available={}, reason={})",
            model_name,
            resolution.effective_device,
            resolution.capabilities.cuda_available,
            resolution.reason
        );
        ...
        let mut builder = Session::builder()?
            ...
            .with_execution_providers(resolution.providers)?
